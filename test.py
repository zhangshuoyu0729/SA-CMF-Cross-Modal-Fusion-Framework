...
from metrics import candidate_box_metrics, summarize_inference_timing
...
def load_gt_boxes_from_labelme(annotation_path):
    """
    Optional LabelMe-style GT box loader.
    This is only used when GT_BOX_DIR is set for candidate-box evaluation.
    """
...
def find_gt_annotation(gt_box_dir, sample_id):
    if gt_box_dir is None:
        return None

    candidates = [
        os.path.join(gt_box_dir, f"{sample_id}.json"),
        os.path.join(gt_box_dir, f"{sample_id}_A.json"),
    ]
...
def predict_multiple_objects(
    ...
    for start_idx in range(0, len(boxes), batch_size):
        ...
        batch_sizes.append(len(batch_boxes))
    ...
    timing_summary = summarize_inference_timing(batch_latencies, batch_sizes)

    timing_info = {
        "num_candidates": total_processed,
        "total_forward_time": timing_summary["total_forward_time"],
        "avg_batch_latency_ms": timing_summary["avg_batch_latency_ms"],
        "avg_sample_latency_ms": timing_summary["avg_sample_latency_ms"],
        "throughput": timing_summary["throughput"],
        "candidate_box_metrics": None
    }

    if gt_boxes is not None:
        selected_boxes = [box for _, _, box in selected]
        timing_info["candidate_box_metrics"] = candidate_box_metrics(
            selected_boxes,
            gt_boxes,
...
def print_timing_summary(timing_list):
    ...
    avg_sample_latency_ms = np.mean([
        item["avg_sample_latency_ms"]
        for item in valid
    ])
    ...
    print(f"Latency benchmark - avg single-sample: {avg_sample_latency_ms:.3f} ms")
    print(f"Latency benchmark - avg batch forward: {avg_batch_latency_ms:.3f} ms")
    print(f"Throughput benchmark - total forward time: {total_forward_time:.6f} s")
    print(f"Throughput benchmark - batched rate: {overall_throughput:.2f} samples/s")
    print("==================================================================")
...
if __name__ == "__main__":
    ...
    for sid in ids:
        ...
        gt_annotation = find_gt_annotation(GT_BOX_DIR, sid)
        gt_boxes = load_gt_boxes_from_labelme(gt_annotation)
        ...
        if timing_info["candidate_box_metrics"] is not None:
            all_candidate_metrics.append(timing_info["candidate_box_metrics"])

        print(
            f"[RESULT] candidates={timing_info['num_candidates']} | "
            f"sample_latency={timing_info['avg_sample_latency_ms']:.3f} ms | "
            f"batch_latency={timing_info['avg_batch_latency_ms']:.3f} ms | "
            f"batched_throughput={timing_info['throughput']:.2f} samples/s"
        )
        ...
        if timing_info["candidate_box_metrics"] is not None:
            box_metrics = timing_info["candidate_box_metrics"]
            print(
                f"[Candidate Box] P={box_metrics['precision']:.4f} | "
                f"R={box_metrics['recall']:.4f} | "
                f"F1={box_metrics['f1']:.4f} | "
                f"TP={box_metrics['tp']} | FP={box_metrics['fp']} | "
                f"FN={box_metrics['fn']}"
            )

    print_timing_summary(all_timing)

    if len(all_candidate_metrics) > 0:
        total_tp = sum(item["tp"] for item in all_candidate_metrics)
        total_fp = sum(item["fp"] for item in all_candidate_metrics)
        total_fn = sum(item["fn"] for item in all_candidate_metrics)
        precision = total_tp / (total_tp + total_fp + 1e-9)
        recall = total_tp / (total_tp + total_fn + 1e-9)
        f1 = 2.0 * precision * recall / (precision + recall + 1e-9)

        print("\n================ Candidate Box Evaluation ================")
        print(f"IoU threshold: {GT_IOU_THRESHOLD}")
