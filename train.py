...
def train_model(vocab_size, max_len, embed_dim,
    ...
    for epoch in range(epochs):
        ...
        prec = cls_metrics["precision_weighted"]
        rec = cls_metrics["recall_weighted"]
        f1 = cls_metrics["f1_weighted"]
        ...
        print(
            f"\n[Epoch {epoch+1}] "
            f"Acc={acc:.4f} | Prec_w={prec:.4f} | Recall_w={rec:.4f} | "
            f"F1_w={f1:.4f} | AUC_wOvR={roc_auc:.4f}\n"
            f" Micro: Prec={cls_metrics['precision_micro']:.4f} | "
            f"Recall={cls_metrics['recall_micro']:.4f} | "
            f"F1={cls_metrics['f1_micro']:.4f}\n"
            f" Macro: Prec={cls_metrics['precision_macro']:.4f} | "
