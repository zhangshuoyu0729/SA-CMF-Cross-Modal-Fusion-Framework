...
def mutual_information_map(img_feat, seq_feat, theta=2.0):
    ...
    img_score = tf.reduce_mean(img_feat, axis=-1)  # [B, Ti]
    seq_score = tf.reduce_mean(seq_feat, axis=-1)  # [B, To]

    u = ecdf(img_score)
    v = ecdf(seq_score)

    u = tf.expand_dims(u, axis=-1)  # [B, Ti, 1]
    v = tf.expand_dims(v, axis=1)   # [B, 1, To]
